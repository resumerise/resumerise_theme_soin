...
* `index.ts`: This is the file that will return the needed HTML to the theme server. You can use it to process some things with your theme first, but we'll talk about that a bit later.
* `resume.hbs`: This is your actual template. This file is sent to the `index.ts` for it to sent to the theme server.
* `style.css`: This is where all the CSS of your project goes. Since the `index.ts` only returns HTML, the contents of this file are put between `<style>` tags in your `resume.hbs` file.
...
The `index.ts` is where all the compiliing of your theme, and neccessary edits will go.
